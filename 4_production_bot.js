    ...
    if (requiresCode) {
        ...
        if (code) {
            ...
            for (const inp of inputs) {
                if (entered >= 6) break;
                await inp.focus(); await inp.type(code[entered], { delay: 200 }); entered++;
            }
            await sleep(1000);
            await tPage.evaluate(() => {
        ...
        try {
            ...
            if (sr.images_local && sr.images_local.length > 0) {
                ...
                if (fs.existsSync(bannerPath)) {
                    ...
                    if (isPro) {
                        log('⚠️ Баннер заблокирован PRO. Пробуем "Image"...');
            ...
            log('Загрузка Подзаголовка и Bio...');
            await tPage.click('button.is-new-block'); await sleep(2500);
            ...
            // ШАГ 4: ССЫЛКИ С ЗАГОЛОВКОМ
            log('Добавление Заголовка Списка...');
            await tPage.click('button.is-new-block'); await sleep(2500);
            await tPage.evaluate(() => Array.from(document.querySelectorAll('button.is-block-button')).find(el => el.innerText?.includes('Текст') || el.innerText?.includes('Text'))?.click());
            await sleep(3500);
