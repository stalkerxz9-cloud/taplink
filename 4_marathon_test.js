...
async function launchNewBrowser() {
    ...
    log(`[БРАУЗЕР] Запуск в ВИДИМОМ РЕЖИМЕ для теста...`);
...
async function getVisualEmail(mailPage) {
    ...
    try {
        for (let i = 0; i < 3; i++) {
            ...
            await sleep(5000); // Даем больше времени на самом старте
            ...
            try {
                ...
                log(`Попытка ${i+1} не удалась, обновляю почту...`);
...
async function registerTaplink(tPage, mailPage, email, password) {
    ...
    // НОВОЕ: Обработка подтверждения "Почта существует"
    await tPage.evaluate(() => {
    ...
    if (requiresCode) {
        ...
        if (code && typeof code === 'string' && code.length === 6) {
            ...
            log('Код не найден или не валиден.');
        }
    }
    ...
    try {
        ...
        await tPage.evaluate(() => {
            ...
            if (cb) cb.click();
            const btn = Array.from(document.querySelectorAll('button, input[type="submit"]'))
                .find(b => /регистр|войти|продолжить/i.test(b.innerText || b.value || ''));
            if (btn) btn.click();
        });
        await sleep(8000);
    } catch(e) {}
...
async function ensureSaved(page) {
    ...
    for (let i = 0; i < 4; i++) {
        const ok = await page.evaluate(() => {
            ...
            const btn = btns.find(b => /сохранить|готово/i.test(b.innerText || ''));
...
// ─── ГЛАВНЫЙ ЦИКЛ ГЕНЕРАЛЬНОЙ РЕПЕТИЦИИ ───────────────────────────────────────
async function run() {
    let showrooms = JSON.parse(fs.readFileSync(dataPath, 'utf-8'));
    
    // ПРЯМО СПРАШИВАЕМ ПЕРЕД НАЧАЛОМ
    const readline = (await import('readline')).createInterface({ input: process.stdin, output: process.stdout });
    const answer = await new Promise(r => {
        readline.question('\n⚠️ СБРОСИТЬ ВЕСЬ ПРОГРЕСС И НАЧАТЬ ВСЕ ЗАНОВО? (y/N): ', ans => r(ans.toLowerCase()));
    });

    if (answer === 'y' || answer === 'д') {
        log('🔄 Сбрасываю прогресс в базе...');
        showrooms.forEach(s => {
            s.taplink_created = false; s.taplink_published = false; s.taplink_url = ''; 
        });
        fs.writeFileSync(dataPath, JSON.stringify(showrooms, null, 2));
    }

    log('🚀 СТАРТ ТЕСТОВОГО МАРАФОНА (2 АККАУНТА)...');
    
    // БЕРЕМ ТОЛЬКО 2 ПЕРВЫХ
    for (let i = 0; i < 2; i++) {
        ...
        try {
            ...
            if (!email) {
                log('❌ КРИТИЧЕСКАЯ ОШИБКА: Почта не получена. Пропускаем шоурум.');
                await browser.close();
                continue;
            }
            ...
            // LINKS
            const templates = [
                { k: 'profile_url', t: 'Official Catalog' },
                { k: 'cars_url', t: 'Luxury Cars in Sale' }
            ];
            for (const item of templates) {
                ...
                if (inps.length >= 3) {
                    await inps[0].type(item.t); // Title
                    await inps[2].type(sr[item.k]); // URL
                }
                await ensureSaved(tPage);
            }
            ...
            log(`✅ УСПЕХ: ${sr.name} полностью настроен.`);
            sr.taplink_published = true;
            fs.writeFileSync(dataPath, JSON.stringify(showrooms, null, 2));

        } catch (e) {
            log(`❌ ОШИБКА НА ${sr.name}: ${e.message}`);
        } finally {
            log(`Закрытие сессии ${sr.name}...`);
            // Оставляем небольшую паузу чтобы посмотреть результат
            await sleep(5000);
            await browser.close();
        }
    }
    log('\n🏁 ГЕНЕРАЛЬНАЯ РЕПЕТИЦИЯ ОКОНЧЕНА. Если всё Ок - запускайте основной марафон!');
    readline.close();
}
