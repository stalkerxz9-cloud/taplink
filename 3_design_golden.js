...
async function run() {
    ...
    log(`🏁 СТАРТ ГЛОБАЛЬНОГО МАРАФОНА (РЕСТАВРАЦИЯ ВЧЕРАШНЕЙ ЗОЛОТОЙ ВЕРСИИ): ${total} шоурумов.`);
    ...
    for (let index = 0; index < total; index++) {
        ...
        try {
            ...
            // =========================================================================
            // НОВАЯ ИНЪЕКЦИЯ ОТ 9 АПРЕЛЯ: ПРОБИВАЕМ ИИ-ШАБЛОН ДЛЯ ВЧЕРАШНЕГО ФЛОУ
            // =========================================================================
            const afterUrl = await page.url();
            if (afterUrl.includes('/templates/')) {
                log('Обнаружен экран выбора шаблона. Физически пробиваем "Пустой шаблон"...');
                ...
                // Обучающее меню: снимаем фокус
                await page.mouse.click(10, 10); await sleep(1000);
                
                // Вчерашняя версия подразумевает клик по .is-new-block снизу.
                // Пустой шаблон вставляет сверху невидимую пустышку аватара, которая сдвинет координаты! 
                // Безопаснее убить её из DOM, чтобы страница была девственно пустой (как вчера):
                try {
                     await page.evaluate(() => {
